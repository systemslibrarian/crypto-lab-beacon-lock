 ...
 *
 *  - text over a surface axe declines to resolve — a `color-mix()`. This lab
 *    has two: the hero's "why it matters" aside is `color-mix(in oklab,
 *    var(--accent) 6%, transparent)`, a translucent tint whose real backdrop
 *    is the page background two levels up, and the mechanism stepper's current
 *    dot is `color-mix(in oklab, var(--accent) 16%, var(--surface-2))`. axe
 *    files both under `incomplete` rather than measuring them.
 *  - SVG text, which takes its ink from `fill` and not from `color`. The
 *    comparison chart's axis labels, tick labels and direct series labels are
 *    all `<text>` painted over grid `<line>`s inside an `<svg>` with no
 *    background of its own. Reading `color` on those reports an inherited
 *    value the glyphs are not painted in.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab has one such rule,
 *    `.cl-hero-sub { opacity: .85 }`, and it is measured here rather than
 *    assumed harmless. The gate this replaces injected a stylesheet to suppress
 *    motion; the same injection idiom elsewhere in this fleet carried
 *    `* { opacity: 1 !important }`, which fabricates a pass for precisely this
 *    case. Nothing is injected here.
 *
 ...
 *    boxes are every `.hexblock` (`overflow-x: auto` around a 96-byte G2 point
 *    or a 576-byte GT element), `.hexblock-tall` (`max-height: 8rem` over the
 *    1152-hex-digit pairing outputs the hexDiff prints), `.tablewrap`,
 *    `.chart-host` and `.labnav-scroll`. Content scrolled past those boxes is
 ...
 *    load-bearing here, not defensive: the comparison chart draws its axis
 *    labels, tick labels and direct series labels as `<text>` over a field of
 *    grid `<line>`s, and its `<svg>` has no background of its own. An
 *    ancestor-only walk would composite those glyphs onto that transparent
 *    background and report a ratio nothing on screen has.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: the hero aside and the
     * stepper's current dot are authored with `color-mix()`, and Chromium
     * reports that to
     ...
     *
     * This page paints no gradients at all — every surface is a flat colour or
     * a flat `color-mix`. The sampling machinery is kept rather than deleted
     * because the failure it prevents is silent: judging a gradient at its
     * worst *stop* is right only when that stop spans the element, and a
     * gradient added later would be quietly mismeasured rather than loudly
     * unsupported. What IS exercised here is the last line of `sampleLayer`'s
     * contract — a non-gradient layer is unmeasurable and paints nothing, which
     * is the correct answer for the only `background-image` on the page, the
     * `url("data:image/svg+xml,…")` chevron on every `<select>`.
     */
     ...
     *
     * A driven scan here walks eight exhibits at once — the vault, the beacon's
     * published-round list, the eight-step mechanism with its 1152-hex-digit
     * pairing comparisons, the chart's SVG labels, the comparison table and the
     * attack log — and every one of those nodes re-walks the same handful of
     * ancestors up to `<body>`. Without the caches the pass re-reads the same
     * computed styles and rects tens of thousands of times, and the gate scans
     * fifty-odd states per configuration. Nothing mutates the DOM during the
     * pass — the beacon is paused for the whole drive — so the cached values
     * cannot go stale.
     */
     ...
     * that is every `.hexblock` and `.hexblock-tall`, every `.tablewrap`,
     * `.chart-host` and `.labnav-scroll`.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This chart is exactly that shape: its grid rules are
      // stroke-only `<line>`s crossing the whole plot, and its three model
      // curves are `<path fill="none">` — `resolve` returns null for the
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it, and it matters here:
      // this lab ships nine `<details class="expert">` blocks. The gate opens
      // each one by clicking its `<summary>` and scans it open AND shut, rather
      // than setting `open = true` on all of them at once and stripping every
      // `[hidden]`, the way the gate it replaces did — producing a document no
     ...
     * glyph, the `✓`/`✗` prefix on each hexdiff summary and trade card, the
     * `→` in the comparison summary, the digit inside each mechanism step dot
     * (whose readable name is the sibling `.stepdot-actor`), and the shared
     * header's hamburger and GitHub marks — would be measured as if each took
     * its element's declared `color`. Note the boundary cuts both ways and
     ...
     *
     * The comparison chart is built with `createElementNS`, so it has no stray
     * text nodes today. The guard still earns its place: `svg()` in `ui/dom.ts`
     * accepts arbitrary children, and a bare string passed to it becomes a text
     * node inside a `<g>` or an `<svg>` — in the DOM, painting nothing. A walk
     * that measured one would report a 1:1 failure describing ink that was
     * never laid down.
     */
